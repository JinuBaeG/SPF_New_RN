...
const SEE_ADMIN_FAQS_QUERY = gql`
  query seeAdminFaqs($offset: Int) {
    seeAdminFaqs(offset: $offset) {
      id
      title
      createdAt
    }
...
export default function AdminFaq({ navigation }: any) {
  const {
    data: noticeData,
    loading: noticeLoading,
    refetch: noticeRefetch,
    fetchMore: noticeFetchMore,
  } = useQuery(SEE_ADMIN_FAQS_QUERY, {
    variables: {
      offset: 0,
    },
      ...
      <FlatList
        ...
        onEndReached={() => {
          return noticeFetchMore({
            variables: {
              offset: noticeData?.seeAdminFaqs?.length,
            },
          });
        }}
